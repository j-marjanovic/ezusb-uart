...
int ezusb_init(unsigned busnum, unsigned devaddr,
  ...
  if (status < 0) {
    debug_print("[ERROR] libusb_init() failed: %s\n",
  ...
  if (libusb_get_device_list(NULL, &devs) < 0) {
    debug_print("[ERROR] libusb_get_device_list() failed: %s\n",
                libusb_error_name(status));
    return -1;
  }
  ...
  if (!dev) {
    debug_print("[ERROR] could not open device\n");
    return -1;
  }
  ...
  if (status < 0) {
    debug_print("[ERROR] libusb_open() failed: %s\n",
                libusb_error_name(status));
    return -1;
  }
  ...
  if (status != LIBUSB_SUCCESS) {
    libusb_close(*device);
    debug_print("[ERROR] libusb_claim_interface failed: %s\n",
                libusb_error_name(status));
    return -1;
  }
...
int ezusb_cmd_read(libusb_device_handle *device, const char *label,
  ...
  if (ezusb_verbose > 1) {
    debug_print("[TRACE] %s, addr 0x%08x len %4u (0x%04x)\n", label, addr,
...
int ezusb_uart_send_buffer(libusb_device_handle *device, char *buffer,
  ...
  debug_print("ezusb_uart_send_buffer, len = %d\n", len);
  ...
  int status = libusb_bulk_transfer(device, LIBUSB_ENDPOINT_OUT | 2, tx_buf, 64,
                                    &transferred, 1000);
  debug_print("  status = %d (%s), transferred = %d\n", status,
...
int ezusb_uart_recv_buffer(libusb_device_handle *device, unsigned char *rx_buf,
  ...
  debug_print("ezusb_uart_recv_buffer\n");
